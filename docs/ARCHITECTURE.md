...
| **Skill maintenance** (daily auto-split + advisory checks) | `skills` (`graduate_seen`/`graduate_kb` params) | `75-skill-maintain.sh`, `np_skill_budget.py`, `np_skill_validate.py`, `np_graduation_detect.py` (flags proven/over-budget lessons to graduate→skill; surfaces to log + `graduation-candidates` marker, never auto-promotes), `np-architecture-freshness.sh` (advisory map-drift), `agents/np-flow-skill-maintain.md` | `specs/2026-06-05-skill-maintenance-routine-design.md` |
| **LLM agent seam** | — | `engine/setup/np_llm_agent.py` (`run_agent` — the shared seam for multi-turn agent loops via `np-llm.sh agent` calls; dispatched from orchestrators; not yet consumed until Phase 10 ports `75-skill-maintain.sh`'s loop logic) | — |
