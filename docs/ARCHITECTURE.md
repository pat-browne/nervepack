...
| **Strategies** (success-driven, advisory) | `strategies` | `strategy-recall.sh`, `memory/strategies/`; distilled by `np-flow-episodic-maintain` from capture `strategies[]` | `specs/2026-06-05-nervepack-vs-sota-evaluation.md` |
...
|---|---|
| **content layer dir names** (`memory/{episodic,playbooks,strategies}`, `wiki/{topics,concepts}/<x>/`) | `np_layer_dir`/`np_layer_roots` (the single resolver — change the subpath here, not at each consumer); the three recall hooks, `playbook-guard`, `73`/`75`, `dashboard/build.py` (`wiki_index`, `learned_counts`), `np-mcp-server.py` `_tool_recall`; the maintain-agent write path (`agents/np-flow-episodic-maintain.md`); the example-layout fixture + `tests/content/test_example_layout.sh` (the anti-drift contract) |
...
| **`np-toggle-lib.sh`** or **`np-content-lib.sh`/`np-layer-lib.sh`** (the resolvers) | their **in-process Python ports** `engine/setup/np_toggle.py` (`enabled`/`param`) and `engine/setup/np_content.py` (`content_dir`/`origin`/`is_explicit`/`team_dir`/`content_layers`/`merge_mode`/`merge_roots`) must stay byte-for-byte equivalent — the long-running MCP server resolves toggles+content **in-process** via these (no bash subprocess per request; `NP_MCP_PURE_PYTHON=0` falls back to shelling to the bash libs). A/B parity is enforced by `engine/setup/tests/mcp/parity/test_toggle_parity.sh` + `test_content_parity.sh` (bash original vs Python port, byte-identical stdout + exit across a footgun table — empty config, missing key, sub-toggle inheritance, CRLF, team modes). Change one side → the parity test goes red until the other matches. First milestone of the git-for-windows-free MCP (overlay `specs/2026-06-30-git-for-windows-free-mcp-design.md`); bash stays the source of truth for the hot-path hooks/crons. |
