...
| **`np-layer-lib.sh`** / **`team.merge`** param | the three recall hooks (`playbook-recall.sh`, `strategy-recall.sh`, `episodic-recall.sh`) that call `np_content_layers`/`np_merge_mode`/`np_merge_roots`; `np-doctor.sh` (reports the resolved mode); `dashboard/build.py` `wiki_index()` (live consumer — shells out to `np_merge_roots`/`np_merge_mode`; fail-open to personal-only). |
