...
5. **Superseded: "Bash for glue, Python for parsing/logic."** The original split
   ...
   same caveat — the bash file stays until phase 10 finishes. Phase 10
   (`np-implement-suggestion.sh` + `np-hook-lib.sh` retirement) is in progress; the
   `np-hook-lib.sh` half is blocked the same way (every `5x`/`6x-install-*.sh` hook
