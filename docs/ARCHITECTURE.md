...
| **`np-toggle-lib.sh`** / **`np-content-lib.sh`·`np-layer-lib.sh`** / **`episodic-match.sh`** (resolvers + recall matcher) | their **in-process Python ports** `engine/setup/np_toggle.py` (`enabled`/`param`), `np_content.py` (`content_dir`/`origin`/`is_explicit`/`team_dir`/`content_layers`/`merge_mode`/`merge_roots`), and `np_episodic_match.py` (`match`) must stay byte-for-byte equivalent — the long-running MCP server resolves toggles+content and matches recall **in-process** via these (no bash subprocess per request; `NP_MCP_PURE_PYTHON=0` falls back to shelling to the bash originals). A/B parity is enforced by `engine/setup/tests/mcp/parity/test_{toggle,content,episodic_match}_parity.sh` (bash original vs Python port, byte-identical stdout across a footgun table — empty config, missing key, sub-toggle inheritance, CRLF, team modes; matcher: header/separator skip, scoring, `sort -rn` tie-break, hyphenated keywords). Change one side → the parity test goes red until the other matches. Part of the git-for-windows-free MCP milestone (overlay `specs/2026-06-30-git-for-windows-free-mcp-design.md`); bash stays the source of truth for the hot-path hooks/crons. Bash-free proof: `engine/setup/tests/mcp/test_bashfree.py` + the `windows-bashfree` CI lane. |
