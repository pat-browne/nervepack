...
| **`toggle-schema.json`** (dashboard param types) | `np_toggle_schema.py` (the loader/validator — keep `load()`/`validate()` stable), `np-dashboard-server.py`'s `/api/toggles`/`/api/toggle` (consumers), `index.html`'s per-type render branch (`bool`/`number`/`enum`/`string`), and its test (`engine/setup/tests/mcp/test_toggle_schema.py`) |
