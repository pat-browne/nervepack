...
| **MCP layer** (model-agnostic surface) | `mcp` (`writes`/`contribute` params) | `engine/setup/np-mcp-server.py` (stdlib stdio JSON-RPC dispatcher), `engine/bin/nervepack-mcp` (launcher), `engine/setup/58-install-mcp.sh` (Claude Code registration), `engine/setup/tests/mcp/test_mcp_server.py`. **Team-layer-aware (Phase 3):** `_tool_recall` merges episodic/playbook/strategy recall across team+personal overlays per `team.merge`. | `specs/2026-06-08-nervepack-mcp-layer-design.md` |
