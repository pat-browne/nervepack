#!/usr/bin/env bash
# PreToolUse hook: match the imminent command/tool against playbook patterns.
# Phase 1: Bash command vs INDEX.md tool_match (gate=ask → confirm; gate=warn → inject).
# Phase 2: non-Bash tool_name vs armed marker + playbook frontmatter tool_name_match.
# Fail-open: any problem → exit 0.
set -uo pipefail
_npl="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/np-toggle-lib.sh"; [[ -r "$_npl" ]] && source "$_npl" && { np_enabled playbooks.guard || exit 0; }
...
fire_gate() {  # $1=gate $2=topic
  ...
  local f="$PB_DIR/$topic.md"
  local body fp
  body="$([[ -f "$f" ]] && grep -E '^\*\*(Symptom|Why|Do|Avoid):' "$f" | tr '\n' ' ')"
  [[ -z "$body" ]] && body="See playbooks/$topic.md"
  fp="$(printf '%s' "${cmd:-${tool_name}:${file_path:-}}" | tr -s '[:space:]' ' ' | sed -E 's/^ +| +$//g' | sha256sum 2>/dev/null | cut -c1-16)"
  np_signal "$sid" "playbook-guard $gate $topic :: $fp"
  if [[ "$gate" == "ask" ]]; then
    jq -nc --arg r "Nervepack playbook '$topic': $body" \
      '{hookSpecificOutput:{hookEventName:"PreToolUse",permissionDecision:"ask",permissionDecisionReason:$r}}'
  else
    jq -nc --arg c "Nervepack playbook '$topic' (past failure pattern): $body" \
...
if [[ -n "$cmd" ]]; then
  while IFS='|' read -r _ topic tool_match gate _rest; do
    ...
    [[ -z "$tool_match" ]] && continue                            # conceptual-only playbook
