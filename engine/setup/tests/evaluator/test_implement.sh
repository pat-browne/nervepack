...
# DEBUG (temporary, remove once the Windows CI hang is diagnosed): run-all.sh
# captures each test's stdout via command substitution, which buffers ALL
# output until the subprocess exits -- so on a genuine hang (never exits,
# eventually killed by CI cancellation), nothing this script echoes ever
# reaches the log. Mirror each marker to a fixed on-disk path instead so a
# companion `if: always()` CI step can cat it even after a cancel/kill.
PROGRESS_FILE="$NP/.ci-debug-implement-progress.log"
progress() { echo "  [progress] $1"; printf '%s\n' "$1" >> "$PROGRESS_FILE"; }
: > "$PROGRESS_FILE"
progress "0: script started"
...
progress "0b: setup complete (repo + stubs + helpers)"
...
progress "6: prompt-injection cap"
...
progress "8: resolution committed clean"
...
progress "9: simulated timeout exit code"
: > "$tmp/resolved.txt"; git -C "$repo" checkout -q "$base" 2>/dev/null
...
progress "9b: genuine subprocess timeout"
: > "$tmp/resolved.txt"; git -C "$repo" checkout -q "$base" 2>/dev/null
...
progress "10: content-overlay fallback"
: > "$tmp/resolved.txt"; git -C "$repo" checkout -q "$base" 2>/dev/null
...
progress "11: engine-only, no content touch"
: > "$tmp/resolved.txt"; git -C "$repo" checkout -q "$base" 2>/dev/null
...
progress "13: both not implementable"
: > "$tmp/resolved.txt"; git -C "$repo" checkout -q "$base" 2>/dev/null
