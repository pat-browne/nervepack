...
# SessionStart hook, trailing `>/dev/null 2>&1 &` — the redirect is load-bearing:
...
jq -e '[.hooks.SessionStart[].hooks[].command] | any(test("np-backcapture-sweep.sh >/dev/null 2>&1 &$"))' "$CLAUDE_SETTINGS" >/dev/null \
