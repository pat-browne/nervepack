#!/usr/bin/env bash
# Asserts np-skill-budget.py scans BOTH engine and overlay skill roots.
set -uo pipefail
...
out="$(python3 "$SETUP/np-skill-budget.py" "$tmp/engine/skills" "$tmp/overlay/skills" 2>/dev/null)"
