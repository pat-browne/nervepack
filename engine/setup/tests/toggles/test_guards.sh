...
out="$(printf '%s' "$(jq -nc '{session_id:"x",prompt:"rename stuff"}')" | EPISODIC_LESSON_DIR="$tmp/lessons" EPISODIC_STATE_DIR="$tmp/st" bash "$S/lesson-recall.sh")"
[[ -z "$out" ]] || { echo "FAIL: lesson-recall ran while off: $out"; exit 1; }
# with it ON, it injects
echo "lessons=on" > "$tmp/local"
out2="$(printf '%s' "$(jq -nc '{session_id:"y",prompt:"rename stuff"}')" | EPISODIC_LESSON_DIR="$tmp/lessons" EPISODIC_STATE_DIR="$tmp/st" bash "$S/lesson-recall.sh")"
