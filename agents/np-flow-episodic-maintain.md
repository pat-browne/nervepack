...
(`~/.cache/nervepack/episodic-inbox/*.jsonl`) into themed `episodic/<topic>.md`
files, compacts oversized ones, regenerates `episodic/INDEX.md`, and
...
**Standing mandate:** pre-authorized to commit + push to the `episodic/`
...
Your job: drain the episodic capture inbox into nervepack's `episodic/` layer.
...
**Where the layers live (post content-cutover):** `episodic/`, `playbooks/`,
`strategies/` (and `wiki/`) now live in the **content repo**, which is your **cwd**
(`72-run-episodic-maintain.sh` already `cd`s there via `np_content_dir`). Read and write
those relative to the cwd; commit + push the cwd repo (Step 6). Do **not** `cd` to the
engine repo.
...
- Pick the best topic. Prefer reusing an existing `episodic/<topic>.md` or a
  `candidate_topics` slug that already maps to one. Check `episodic/INDEX.md`
...
- If it doesn't exist, create it with the frontmatter in
  `episodic/README.md` (`name`, `kind: episodic`, `last_updated`, `wiki`,
...
For any `episodic/<topic>.md` over ~200 lines:
...
retire it: `git mv episodic/<topic>.md archive/episodic/<topic>.md`.
...
### 5. Regenerate `episodic/INDEX.md`

Rewrite it from the current `episodic/*.md` files, preserving the header from
`episodic/README.md`'s format. One row per topic:
...
1. Cluster recurring struggles by their failure pattern. For each cluster,
   create or refresh `playbooks/<topic>.md` using the format in
   `playbooks/README.md` — synthesize **Symptom / Why / Do / Avoid** from the
   struggle items.
...
4. Regenerate `playbooks/INDEX.md` (columns `topic | tool_match | gate |
...
1. Cluster recurring strategies by approach. For each cluster, create or refresh
   `strategies/<topic>.md` using the format in `strategies/README.md` — synthesize
...
4. Regenerate `strategies/INDEX.md` (columns `topic | topic_triggers | seen`),
...
If `episodic/` changed — commit + push **the cwd repo** (the content repo; do NOT
`cd` away):
...
_paths="episodic archive/episodic playbooks archive/playbooks strategies archive/strategies"
git add $_paths
...
If `episodic/` did not change, do not commit and do not clear the inbox.
