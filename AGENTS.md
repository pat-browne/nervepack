...
| `docs/GETTING-STARTED.md` | First-time-user walkthrough: clone → onboard → verify. | Yes |
| `docs/CONTENT-OVERLAY.md` | How to configure a personal (and optional team) content overlay — where skills/wiki/memory/metrics actually live. | Yes |
| `INDEX.md` | Auto-generated skill index — **read this before adding a new skill**. In a split layout the merged engine+overlay catalog is written to the overlay (`$NP_CONTENT_DIR/INDEX.md`) for local discovery; the copy committed to the *engine* lists engine skills only (publishable surface — pii-guarded). | **No — regenerated by `cli.py setup link-skills` (which runs `generate-index`)** |
...
| `engine/` | Parent dir for the reusable machinery — `engine/setup/` (hooks, crons, libs, tests), `engine/onboard/` (host contract), `engine/bin/` (MCP launcher), `engine/nervepack_engine/` (the Python package behind `cli.py` — hooks, toggles, model seam), `engine/pyproject.toml`. `dashboard/` (code), `skills/` (engine skills), and `agents/` stay at repo root; the content layers live in the overlay. | Yes — machinery |
...
| `change-specs/<branch-slug>.md` | **Engine change specs — a different thing from the overlay's design specs above.** A per-branch, CI-readable governance artifact (schema, status lifecycle, blast radius) that `spec-guard` CI enforces against the diff. Lives here because CI can only read this repo, not the private overlay. See `change-specs/README.md` for the full contract and the distinction from `docs/superpowers/specs/`. | Yes — one per governed branch, per `change-specs/TEMPLATE.md` |
...
5. **Verify before deploying** (a disciplined skill-authoring/verification pass — e.g.
   ...
   add-skill steps (plugin.json → `cli.py setup link-skills` → commit).
...
4. Run `python3 engine/nervepack_engine/cli.py setup link-skills` — its prune pass
   removes the dangling symlink and regenerates `INDEX.md`.
